...
def distorted_bounding_box_crop(image,
                                ...
                                aspect_ratio_range=(0.65, 1.5),#0.9, 1.1
                                area_range=(0.8, 1.0),#0.1
                                max_attempts=200,
                                clip_bboxes=True,
                                scope=None):
...
def preprocess_for_train(image, labels, bboxes,
    ...
    with tf.name_scope(scope, 'ssd_preprocessing_train', [image, labels, bboxes]):
        ...
        one = tf.constant(1.)
        bboxes_0 = math_ops.maximum(bboxes[:,0], zero)
        bboxes_1 = math_ops.maximum(bboxes[:,1], zero)
        bboxes_2 = math_ops.minimum(bboxes[:,2], one)
        bboxes_3 = math_ops.minimum(bboxes[:,3], one)
        bboxes = tf.stack([bboxes_0, bboxes_1, bboxes_2, bboxes_3], axis=-1)
